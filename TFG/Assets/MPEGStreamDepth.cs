...
public class MJPEGStreamDepth : MonoBehaviour
{
    public RawImage rawImage; //Donde se va a colocar la imagen
    public string URL = "http://192.168.1.30:5000/depth_stream";//web donde está el video
    ...
    IEnumerator Obtener_Video()
    {
        ...
        while (true) //leer continuamente el stream
        {
            ...
            depthStream.Write(buffer, 0, bytesleidos);
            //Convierte el VideoStream a un array y lo guarda
            byte[] videobytes = depthStream.ToArray();
